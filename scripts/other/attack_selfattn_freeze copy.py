...
def compute_freeze_loss(model, x_padded, condition, T_tok, num_attack_layers=None, skip_latent = False):
    ...
    #                     Without them, recomputation saves fewer tensors → mismatch.
    #   • detach_hook   – changes the tensor passed between block[cutoff-1] and
    #                     block[cutoff].  Without it, the recomputed output has a
    #                     grad_fn and a different save-set → mismatch.
    #
    # The caller (pgd) must invoke _cleanup() after cost.backward() returns.
    # Extra appends to loss_terms during recomputation are harmless: the loss
    # scalar was already computed from the original-forward entries.
    ...
    return torch.stack(loss_terms).mean(), _cleanup
...
def main():
    ...
    if args.ckpt_path is None: args.ckpt_path - ckpt_path
    if args.experiment_name is None: args.experiment_name = experiment_name
